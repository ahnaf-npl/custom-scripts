...
(function () {
  ...
  function translateComboboxButtons(root) {
    root.querySelectorAll("button, [role='combobox']").forEach((element) => {
  ...
  function translateFieldRoots(root) {
    Object.keys(TRANSLATIONS.labelsByFieldCode).forEach((fieldCode) => {
      const direct = root.querySelector('[data-field-code="' + CSS.escape(fieldCode) + '"]');
  ...
  function collectMissingJapanese(root) {
    ...
    root.querySelectorAll("option, [role='option'], button, label, span, p, div").forEach((element) => {
      if (!isElementVisible(element)) return;
      const text = normalizeText(element.innerText || element.textContent);
  ...
  function applyTranslations(reason) {
    state.runCount += 1;
  ...
  function installEventHooks() {
    ...
    document.addEventListener("focusin", () => schedule("focusin"), true);
    document.addEventListener("keyup", () => schedule("keyup"), true);
  }
