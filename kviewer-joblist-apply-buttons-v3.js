...
(function () {
  ...
  function cleanupMisplacedDetailButtons() {
    document.querySelectorAll("aside .kv-job-detail-toolbar").forEach((toolbar) => {
      toolbar.querySelectorAll("[data-kv-job-action='apply']").forEach((button) => {
        button.remove();
      });
  ...
  function findBackElement(root) {
    ...
    const links = Array.from(scope.querySelectorAll("a, button, [role='button']"))
    ...
    return links.find((element) => {
      ...
      return (
        ...
        (href.includes(CONFIG.listPath) && Boolean(element.closest(".kv-detail")))
      );
    }) || null;
  }
  ...
  function openApplyModal(jobCode, triggerElement) {
    ...
    [
      ...
      "Klik tombol DAFTAR KE WHATSAPP setelah tombol aktif.",
    ...
    codeHint.className = "kv-job-code-hint";
    codeHint.textContent = "Opsional: klik kode ini jika ingin menyalinnya manual.";

    const codeButton = document.createElement("button");
    codeButton.type = "button";
    codeButton.className = "kv-job-code-copy";
    codeButton.innerHTML =
      '<span class="kv-job-code-spacer" aria-hidden="true"></span><span class="kv-job-code-value"></span><span class="kv-job-code-icon-wrap"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-copy-icon lucide-copy" aria-hidden="true"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg></span><span class="kv-job-code-status">Klik area kode untuk copy</span>';
    codeButton.querySelector(".kv-job-code-value").textContent = cleanJobCode;
    codeButton.addEventListener("click", () => copyJobCode(cleanJobCode, codeButton));
  ...
  function setCopyStatus(button, message, success) {
    ...
    window.setTimeout(() => {
      ...
      status.textContent = "Klik area kode untuk copy";
    }, 2400);
  }
